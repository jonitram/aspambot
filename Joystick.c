...
int minutes_duration = 60 * minutes;
...
static inline void do_steps(const command_t* steps, uint16_t steps_size, USB_JoystickReport_Input_t* const ReportData, State_t nextState) {
	if (iterations_passed >= minutes_duration) {
		return;
	}
...
void GetNextReport(USB_JoystickReport_Input_t* const ReportData) {
	...
	switch (state)
	{
		...
		case BREATHE:
			do_steps(wake_up_hang, ARRAY_SIZE(wake_up_hang), ReportData, SPAM_A);
			break;
		
		case SPAM_A:
			do_steps(spam_a_steps, ARRAY_SIZE(spam_a_steps), ReportData, SPAM_A);
			break;
